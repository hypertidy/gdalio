...
* `gdalio_format_source()` a helper to *print out the code you can run* to define formatters for spatial package types (raster, stars, terra, spatstat). 
...
Say we don't set a grid at all, just go a default. Currently gdalio has a default that is the entire extent 
of the source, but downsampled to *about* a limit for the shorter side. 
. 
This means we can read from any source and we'll get something (though, it might take some time like a minute or two from rasters tens or hundreds of thousands of pixels per side if it doesn't contain overviews). 
